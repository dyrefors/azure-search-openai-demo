...
class ChatReadRetrieveReadApproach(Approach):
    ...
    def get_search_query(self, response: Response, default_query: str) -> str:
        ...
        try:
            # Handle the custom filename/project lookup tools before falling back to the
            # standard search_sources handling, which only understands "search_query".
            for item in response.output:
                ...
                try:
                    ...
                    continue
                if item.name == "search_by_filename":
                    filename = arguments.get("filename", "")
                    if filename:
                        return f"filename:{filename}"
                if item.name == "search_project":
                    project_code = arguments.get("project_code", "")
                    if project_code:
                        return f"filename:{project_code}.json"
